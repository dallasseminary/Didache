...
void Page_Load() {
	...
	try {
		...
		if (useLocalFiles)
		{
			...
			string videoInfoXmlPath = Server.MapPath("~/playerfiles/" + courseCode + Path.DirectorySeparatorChar + "titles" + Path.DirectorySeparatorChar + language + ".xml");
		...
		foreach (string video in videoNumbers)
		{
			try
			{
				...
				if (useLocalFiles) {
					string transcriptXmlPath = Server.MapPath("~/playerfiles/"+ courseCode + "/transcripts/" + language + "/" + courseCode + "_u" + unitNumber.PadLeft(3, '0') + "_v" + video.PadLeft(3, '0') + "_transcript.xml");
