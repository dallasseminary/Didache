$(document).ready(function () {
	...
	// courses
	createAdminArea('#courses-list',
		'#course-create', '#course-editor', { title: 'Course Editor', height: 300, width: 550 }, '.course', 'courseid', loadCourse, '.course-edit', function() {},
		null, null);

	/*
	// units/tasks
	createAdminArea('#course-units',
	'#add-course-unit', '#unit-editor', { title: 'Unit Editor', height: 300, width: 550 }, '.course', 'courseid', loadCourse, '.course-edit', 
	null, null);

	// files
	createAdminArea('#course-file-groups',
	'#add-file-group', '#file-group-editor', { title: 'File Group Editor', height: 300, width: 550 }, '.course', 'courseid', loadCourse, '.course-edit', 
	null, null);

	// students
	createAdminArea('#courses-user-groups',
	'#add-user-group', '#user-group-editor', { title: 'User Group Editor', height: 300, width: 550 }, '.course', 'courseid', loadCourse, '.course-edit',
	null, null);
	*/

	function createAdminArea(listName,
			createParent, parentDialog, parentDialogOptions, parentRowClass, parentID, loadParentFunction, parentEditButton, saveParent,
			childDialog, childDialogOptions) {


		// setup parent dialog
		var defaultParentDialogOptions = {
			autoOpen: false,
			height: 300,
			width: 550,
			modal: true,
		...
		$.extend(defaultParentDialogOptions, parentDialogOptions);
		$(parentDialog).dialog(parentDialogOptions);

		// create button
		$(createParent).click(function (e) {
			e.preventDefault();

			clearEditor(parentDialog);
		...
		// parent editing
		$(listName).delegate(parentEditButton, 'click', function (e) {
			e.preventDefault();

			var parentID = $(this).closest(parentRowClass).data('courseid');

			loadParentFunction(parentID, function (data) {
				fillEditor(parentDialog, data);
		...
		// inline editing
		$(listName).delegate('input', 'change', function () {
			var 
				row = $(this).closest('tr'),
				course = {
